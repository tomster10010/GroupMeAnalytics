...
def prepare_analysis_of_group(groups_data, group_id):
    ...
    while True:
        #pick a user to display
        user_id = display_names(user_id_mapped_to_user_data)
        display_data_user(user_id_mapped_to_user_data, user_id)
        raw_input("Press enter for another user")
    

    #this line displays the data to the user
    #display_data(user_id_mapped_to_user_data)

def display_names(user_id_mapped_to_user_data):

    i = 0
    for key in user_id_mapped_to_user_data:
        print(str(i) + ' ' + user_id_mapped_to_user_data[key][0] + ' '+str(user_id_mapped_to_user_data[key][1]))
        i+=1
    user_number = int(raw_input("Enter the number of the user you would like to analyze:"))
    j = 0
    for key in user_id_mapped_to_user_data:
        if(j == user_number):
            return key
...
def display_data(user_id_mapped_to_user_data):
    ...
    for key in user_id_mapped_to_user_data:
        print(user_id_mapped_to_user_data[key][0].strip() + ' Data:')
...
def display_data_user(user_id_mapped_to_user_data, key):
